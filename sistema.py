...
db = mysql.connect(host="localhost",user="root",password="",database="vikoakv2")
...
def main():
    while 1:
        ...
        if user_option == "1":
            ...
            student = login(username, password, privilege)
            student.auth()
            ...
            password = input(str("Password : "))
            teacher = login(username, password, privilege)
            teacher.auth()
            ...
            password = input(str("Password : "))
            admin = login(username, password, privilege)
            admin.auth()
            admin_session()
        else:
